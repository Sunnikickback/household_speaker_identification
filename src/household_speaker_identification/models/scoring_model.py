...
class ScoringModel(nn.Module):
    ...
    def forward(self, emb1, emb2):
        ...
        #masked input dropout
        indices = torch.randperm(self.in_features)[:int(self.input_dropout_rate * self.in_features)]
        emb1[indices] = 0
        emb2[indices] = 0

        emb1_after_adaptation_net = self.adaptation_net(emb1)
        emb2_after_adaptation_net = self.adaptation_net(emb2)
